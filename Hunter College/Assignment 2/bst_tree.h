...
/* New */
     ...
     */	
    void displayValue(const Comparable& x, BinaryNode *t) const {
        if(t == nullptr) {
            cout << "Not Found" << endl;
        }
        else if(t->element < x) {
     ...
     * @returns the number 
     */
    int nodeTracker(BinaryNode * node) const {
        if (node == nullptr) return 0;       
        else 
            return (nodeTracker(node->left) + nodeTracker(node->right) + 1);
        }
     ...
     */
    double find(const Comparable& x, BinaryNode * t, int calls_){
   	calls_++;
    	if(t == nullptr) 
    		return 1;
    	else if (x < t->element) 
     ...
     */    
    double depth(BinaryNode* t, int depth_counter_){
